...
function listProSold(start_day, end_day, limit) {
     if (limit == null || limit == '') {
          limit = 10;
     }
     if (limit < 1) {
          ...
          $.ajax({
               ...
               method: "POST",
               data: { start_day: start_day, end_day: end_day, limit: limit },
               success: function (response) {
...
function listProSoldCate(start_day, end_day, selectCategory, limit) {
     if (limit == null || limit == '') {
          limit = 10;
     }
     if (limit < 1) {
          ...
          $.ajax({
               ...
               method: "POST",
               data: { start_day: start_day, end_day: end_day, selectCategory: selectCategory, limit: limit },
               success: function (response) {
                    $('#listProSold').append(response);
               }
          })
...
function listProSoldAllCate(start_day, end_day, limit) {
     if (limit == null || limit == '') {
          limit = 10;
     }
     if (limit < 1) {
...
$(document).ready(function () {
     ...
     $('#statistics').click(function () {
          ...
          if (selectCategory == -1) {
               totalSum(start_day, end_day);
               $('#listProSold').html(``);
               listProSold(start_day, end_day, limit);
               $.ajax({
                    url: "statistics.php/displayChart",
                    method: "POST",
                    data: { start_day: start_day, end_day: end_day },
               ...
               $('#listProSold').html(``);
               listProSoldAllCate(start_day, end_day, limit);
          }
          else {
               let totalSum = 0;
               $.ajax({
               ...
               $('#listProSold').html(``);
               listProSoldCate(start_day, end_day, selectCategory, limit);
          }
     });
});